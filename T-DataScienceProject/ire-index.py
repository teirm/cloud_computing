...
# create data structure for inverted index: dictonary that contains
# dictionaries
list = {}
list['words'] = {}
list['total#Docs'] = 0
...
for file in dirs:
    ...
    if file != path + '.DS_Store':
        ...
        for token in tokens:
            ...
            if token in list['words']:
                #print(token + ' exists')

                # check if doc already in word's docList
                if file in list['words'][token]['docList']:
            ...
            list['words'][token]['wordDocCount'] = len(
                list['words'][token]['docList'])
            # print(token)
