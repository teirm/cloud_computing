...
def submit_mr_index(book_dir="../books"):
    """
    ...
    cnt_cmd_string = '''hadoop jar /lib/hadoop/share/hadoop/tools/lib/hadoop-streaming-2.7.3.jar'''\
        ''' -mapper "python ../map_reduce/count_mapper.py" -reducer "python ../map_reduce/count_reducer.py"'''\
        ''' -input "{}" -output "./output"'''.format(book_dir)

    subprocess.run(shlex.split(cnt_cmd_string))

    # make run2:
    # runs inverted index mapper and reducer
    inv_cmd_string = '''hadoop jar /lib/hadoop/share/hadoop/tools/lib/hadoop-streaming-2.7.3.jar'''\
...
def submit_mr_rar(keywords, num_results):
    ...
    # make search:
    srch_cmd_string = '''hadoop jar /lib/hadoop/share/hadoop/tools/lib/hadoop-streaming-2.7.3.jar'''\
    ...
    # make top_n:
    top_cmd_string = '''hadoop jar /lib/hadoop/share/hadoop/tools/lib/hadoop-streaming-2.7.3.jar'''\
...
# the searching -- Cyrus
def submit_spark_index(book_dir="../books"):
    """
    ...
    cmd_string = """spark-shell -i 
                ../spark/create_index.scala 
                --conf spark.driver.extraJavaOptions="-D{}"
                    """.format(book_dir)
